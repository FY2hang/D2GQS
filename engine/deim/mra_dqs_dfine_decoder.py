...
__all__ = ['DQSDFINETransformer_MRA']
...
@register()
class DQSDFINETransformer_MRA(nn.Module):
    ...
    def _get_decoder_input(self,
        ...
        # 选择top-k查询
        enc_topk_memory, enc_topk_logits, enc_topk_anchors, mra_hard_mask = self._select_topk(output_memory, enc_outputs_logits, densityMapMemory, anchors, self.num_queries)
        ...
        return content, enc_topk_bbox_unact, enc_topk_bboxes_list, enc_topk_logits_list, enc_outputs_logits, mra_hard_mask
    ...
    def _select_topk(self, memory, outputs_logits, densityMapMemory, outputs_anchors_unact, topk):
        ...
        if self.query_select_method == 'default':
            ...
            # -----------------------------------------------------------
                ...
                mra_hard_mask = (density_processed > adaptive_mu).float()
        ...
        return topk_memory, topk_logits, topk_anchors, mra_hard_mask
    ...
    def forward(self, feats, targets=None):
        ...
        # =========================================================
        # 3. [核心手术] 调用 CGFE 进行特征修复
        # =========================================================
        # 只有在 (开启CGFE) AND (密度图存在) 时才执行
        # if self.use_cgfe and self.CGFE is not None and densityMap is not None:
            
        #     # --- 准备数据 A: 投影后的 Backbone 特征 (细节源 B) ---
        #     # 我们需要未展平的、通道数已对齐到 256 的多尺度特征图
        #     # 这保留了最原始的纹理细节 (Inductive Bias)
        #     projected_backbone_feats = []
        #     for i, feat in enumerate(feats):
        #         # 对应层级的投影层 (例如把 512->256, 1024->256)
        #         # self.input_proj 是模型自带的 ModuleList
        #         if i < len(self.input_proj):
        #             proj = self.input_proj[i](feat)
        #             projected_backbone_feats.append(proj)
            
        #     # --- 执行 CGFE 融合 ---
        #     # 输入三要素:
        #     # 1. backbone_feats -> 提供清晰纹理 (Backbone)
        #     # 2. encoder_memory -> 提供全局语义 (Encoder)
        #     # 3. density_map    -> 提供位置先验 (Density)
        #     # 注: 密度图的多尺度缩放现在由 CGFE 内部自动处理
        #     enhanced_memory = self.CGFE(
        #         backbone_feats=projected_backbone_feats,
        #         encoder_memory=memory,
        #         density_map=densityMap,
        #         spatial_shapes=spatial_shapes
        #     )
            
        #     # 用增强后的特征替换原有 Memory，进入后续流程
        ...
        # 获取解码器输入
        init_ref_contents, init_ref_points_unact, enc_topk_bboxes_list, enc_topk_logits_list, enc_outputs_logits, mra_hard_mask = \
            self._get_decoder_input(memory, spatial_shapes, denoising_logits, denoising_bbox_unact, densityMap)

        # =========================================================
        # 🌟 2. 执行 MRA 物理切断：应用由 DQS 计算出的分布掩码
        # =========================================================
        if mra_hard_mask is not None:
            # mra_hard_mask 目前的 shape 是 [B, 1, H_low, W_low] (取决于你的密度图来源层级)
            # 我们需要将其下采样/平展，以对齐 Encoder 多尺度 memory 的长度
            mra_mask_flat = []
            for idx, s in enumerate(self.feat_strides):
                if idx == 0:
                    pooled_mask = nn.MaxPool2d(kernel_size=s, stride=s)(mra_hard_mask)
                else:
        ...
        # =========================================================
        # # =========================================================
        # # 🌟 调试：可视化 MRA 作用后的特征图 (类似 densityMap 的可视化)
        # # =========================================================
        # if not self.training: # 只在推理/测试阶段生成，避免减慢训练
        #     # 1. 找到 P2 层在 memory 中的范围 (假设 P2 是第 0 层)
        #     h_p2, w_p2 = spatial_shapes[0]
        #     len_p2 = h_p2 * w_p2
            
        #     # 2. 截取经过 MRA 过滤后的 P2 特征: [B, L_p2, C]
        #     mra_feat_p2 = memory[:, :len_p2, :]
            
        #     # 3. 计算特征强度：在通道维度取平均，得到 [B, L_p2]
        #     # 如果 MRA 生效，背景区域的强度应该接近 0
        #     mra_intensity = mra_feat_p2.abs().mean(dim=-1)
            
        #     # 4. 还原为 2D 尺寸: [B, H, W]
        #     mra_viz_map = mra_intensity.view(-1, h_p2, w_p2)
            
        #     # 5. 调用原有的可视化函数 (参考你注释掉的那几行)
        #     # 我们取 batch 中的第一个样本 [0]
        #     viz_data = mra_viz_map[0].cpu().detach().numpy()
            
        #     # 归一化处理，方便观察 (让最亮的地方对应 1.0)
        #     viz_data_norm = viz_data / (viz_data.max() + 1e-12)
            
        #     # 使用你项目自带的可视化工具函数
        #     visualize_density_map_only(viz_data_norm, 'mra_feature_p2.png')
            
        #     # 如果你想看未归一化的原始强度（确认是否真的变成了0）
        #     # visualize_density_map_only(viz_data, 'mra_raw_p2.png')
        # # =========================================================

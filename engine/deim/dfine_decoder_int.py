...
class DynamicManifoldIntegral(nn.Module):
    """
    密度扭曲的动态流形积分器 (Density-Warped Dynamic Manifold Integral)
    替代原版静态 Integral，实现基于局部密度的回归空间自适应。
    """
    def __init__(self, reg_max=32):
        super(DynamicManifoldIntegral, self).__init__()
        self.reg_max = reg_max

    def generate_dynamic_basis(self, local_density_1d, base_project):
        """
        根据局部 1D 密度场，动态扭曲物理坐标基底
        """
        base_project = base_project.to(local_density_1d.device)
        # 加入平滑项防止背景绝对 0 导致 CDF 崩溃
        smooth_density = local_density_1d + 1e-4 
        
        # 计算累积密度函数 (CDF)
        cdf = torch.cumsum(smooth_density, dim=-1)
        cdf_max = cdf[..., -1:]
        normalized_cdf = cdf / cdf_max # [B, L, 4, 33] 归一化到 0~1
        
        # 扭曲基底: 密度高的地方 CDF 上升快，物理距离被挤压
        dynamic_project = base_project.view(1, 1, 1, -1) * normalized_cdf
        # ===== 🚨 插入调试代码 3 =====
        if torch.rand(1).item() < 0.01:
            # 挑出 Batch 里的第一个 Query 的左边界（index 0）看看
            original_scale = base_project.view(-1)
            warped_scale = dynamic_project[0, 0, 0, :] # 取出第一个预测框的左边基底
            
            # 计算两把“尺子”的绝对差值
            diff_sum = torch.abs(warped_scale - original_scale).sum()
            # print(f"[DEBUG] 🌌 空间扭曲发生! 扭曲程度(差值总和): {diff_sum.item():.4f}")
            
            # if diff_sum.item() == 0:
            #     print("[DEBUG] ⚠️ 警告：CDF 扭曲无效，空间没有发生形变！")
        # ==============================
        
        return dynamic_project

    def forward(self, x, base_project, local_density_1d=None):
        shape = x.shape
        B = shape[0]
        L = shape[1] if len(shape) > 2 else 1 # 兼容不同维度的输入
        
        # 1. 计算 32 个 bin 的概率
        prob = F.softmax(x.reshape(-1, 4, self.reg_max + 1), dim=-1) # [B*L, 4, 33]
        
        # 2. 生成动态回归基底
        if local_density_1d is not None:
            # ========================================================
            # 🚨 强制验证：如果你在终端测 FLOPs 时看到了这句话，
            # 说明计算图成功追踪到了你的新模块，FLOPs 绝对被算进去了！
            # if not self.training and not torch.is_grad_enabled():
            #      print("\n[FLOPs Profiling] 🚀 正在测算密度流形积分器的计算量...")
            # ========================================================
            dynamic_project = self.generate_dynamic_basis(local_density_1d, base_project) # [B*L, 4, 33]
            # 3. 动态点乘积分
            expected_coords = torch.sum(prob * dynamic_project, dim=-1) # [B*L, 4]
        else:
            # if not self.training and not torch.is_grad_enabled():
            #      print("\n[FLOPs Profiling] ❌ 警告：测算计算量时走入了降级分支！")
            expected_coords = F.linear(prob.reshape(-1, self.reg_max + 1), base_project.to(x.device)).reshape(-1, 4)
            
        return expected_coords.reshape(list(shape[:-1]) + [-1])
    ...
    def forward(self,     
        ...
        for i, layer in enumerate(self.layers):
            ...
            if densityMap is not None:
                # 1. 探针原点：D-FINE 每一层的偏移量都是基于 ref_points_initial 算的
                # 所以我们沿着初始框向四周发射探针
                current_boxes = ref_points_initial 
                
                # 2. 射出 4 条探针，获取边界上的密度分布
                local_density_1d = extract_1d_density_rays(densityMap, current_boxes, self.reg_max)
                
                # 3. 动态流形积分：用扭曲的基底去算距离
                distances = integral(pred_corners, project, local_density_1d=local_density_1d)
                if not self.training and not torch.is_grad_enabled() and i == 0:
                    print(f"\n[FLOPs Profiling] 🚀 成功触发第 {i} 层密度流形回归算子计算！")
            else:
                # 降级：没有密度图时，用原版静态直尺积分
                distances = integral(pred_corners, project)
            inter_ref_bbox = distance2bbox(ref_points_initial, distances, reg_scale)    

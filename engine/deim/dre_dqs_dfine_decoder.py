...
@register()
class DREDQSDFINETransformer(nn.Module):
    ...
    def __init__(self,
        ...
        # # 1. 记录 stride (为了后续 _get_decoder_input 对齐)
        # self.density_stride = 4
        # # 2. 实例化 LightDMG
        # # 现在的 LightDMG (其实内部是 HybridScaleDMG 的逻辑) 只需要两个参数：
        # # - chs (通道数) -> self.hidden_dim
        # # - scale_factor -> 1 (保持 Stride 4，不进行上采样)
        # self.LDMG = LightDMG(self.hidden_dim, scale_factor=1)
        ...
        # # 2. 控制 MultiScaleFeature (它是辅助 LDMG/CGFE 的)
        # # 如果 CGFE 没开，通常这个也不需要 (或者看你逻辑依赖)
        ...
        if self.use_ldmg and self.use_cgfe:
            ...
            self.adre_encoders = nn.ModuleList([
            ...
            self.adre_encoders = None

        # # ------------------------------------------------------------------
        # # [修改 1] 初始化 Focus Module 和 计数回归参数 (Scale/Bias)
        # # ------------------------------------------------------------------
        # self.focus_module = AdaptiveFocusModule(threshold=0.3, temp=20.0, gamma=2.0)  # 修改1：密度图
        # # # 初始值 scale=100.0, bias=0.0，让模型自己去学真实的倍率
        # # self.count_scale = nn.Parameter(torch.tensor(100.0)) 
        # # self.count_bias = nn.Parameter(torch.tensor(0.0))
        # # # ------------------------------------------------------------------
    ...
    def _get_encoder_input(self, feats: List[torch.Tensor]):
        ...
        for i, feat in enumerate(feats):
            # 获取当前特征的实际通道数
            actual_in_channels = feat.shape[1]
            
            # --- 🌟 修复后的判断逻辑 ---
            # 1. 检查是否是 Identity (如果是，说明原本期望的就是 hidden_dim)
            if isinstance(self.input_proj[i], nn.Identity):
                expected_in_channels = self.hidden_dim
            # 2. 检查是否是 Sequential (Conv + BN)
            elif isinstance(self.input_proj[i], nn.Sequential):
                expected_in_channels = self.input_proj[i][0].in_channels
            # 3. 检查是否直接带属性 (Conv)
            elif hasattr(self.input_proj[i], 'in_channels'):
                expected_in_channels = self.input_proj[i].in_channels
            else:
                # 实在拿不到就降级处理
                expected_in_channels = -1 
            
            # 判断逻辑：如果特征已经是 256 (hidden_dim)，直接通过
            if actual_in_channels == self.hidden_dim:
    ...
    def _get_decoder_input(self,
                           ...
                           densityMap=None):
        # 准备解码器输入
        if self.training or self.eval_spatial_size is None or self.anchors.shape[1] != memory.shape[1]:
    ...
    def _select_topk(self, memory, outputs_logits, densityMapMemory, outputs_anchors_unact, topk):
        ...
        if self.query_select_method == 'default':
            ...
            # -----------------------------------------------------------
                ...
                # [Step B] 提取全局与局部的统计分布特征 (Statistical Moments)
                ...
                # 1. 局部特征: 前 K 个高密度目标自身的均值与方差
                topk_mean = val_k.mean(dim=1).view(-1, 1, 1, 1)
                topk_std = val_k.std(dim=1).view(-1, 1, 1, 1)

                # 2. 全局特征: 整张图的背景底噪均值
                global_mean = density_flat.mean(dim=1).view(-1, 1, 1, 1)

                # [Step C] 完全自适应计算边界 \mu
                # 物理逻辑：
                # 理想的阈值应该在 "前景目标的平均水平" 之下，以保证微小目标不被杀掉。
                # 我们设定 mu 为 "Top-K均值 下方一个标准差"。
                # 同时，利用 global_mean 兜底，保证阈值绝对不会低于背景底噪。
                adaptive_mu = topk_mean - topk_std
                adaptive_mu = torch.max(adaptive_mu, global_mean) 

                # [Step D] 完全自适应计算斜率 k (温度系数)
                # 物理逻辑：
                # 目标的方差 (topk_std) 越小，说明目标密度越均匀，需要极其锋利的门控 (大 k) 进行一刀切；
                # 目标的方差 (topk_std) 越大，说明包含极强和极弱的混合目标，需要平滑的门控 (小 k) 给予弱目标喘息空间。
                adaptive_k = 1.0 / (topk_std + 1e-4)
                # 将 k 限制在防止梯度爆炸的合理安全区间
                adaptive_k = torch.clamp(adaptive_k, min=2.0, max=15.0)

                # [Step E] 弹性 Sigmoid 门控
    ...
    def forward(self, feats, targets=None):
        ...
        if self.use_cgfe and self.adre_encoders is not None and densityMap is not None:
            B = memory.size(0)
            enhanced_feats = []
            idx = 0
            
            # --- 1. 提前计算自适应阈值 adaptive_mu (复用你的高阶统计算法) ---
            # 这里的 adaptive_mu 代表当前图像前景的"绝对物理分界线"
            density_processed = densityMap ** (1 / self.densitymap_temperature)
            density_processed = density_processed / (density_processed.max() + 1e-12)
            density_flat = density_processed.view(B, -1)
            
            # 找到 Top-K 的值来计算统计特征 (假设最多看 1500 个高密度点)
            k_safe = min(1500, density_flat.shape[1]) 
            val_k, _ = torch.topk(density_flat, k=k_safe, dim=1)
            
            topk_mean = val_k.mean(dim=1).view(-1, 1)
            topk_std = val_k.std(dim=1).view(-1, 1)
            global_mean = density_flat.mean(dim=1).view(-1, 1)
            
            # 自适应计算边界 (这正是你写的黄金公式)
            adaptive_mu = topk_mean - topk_std
            adaptive_mu = torch.max(adaptive_mu, global_mean) # [B, 1]
            # -----------------------------------------------------------

            # --- 2. 多尺度特征逐层进行 物理裁剪 + 区域 Attention ---
            for i, (h, w) in enumerate(spatial_shapes):
                ...
                # B. 还原成 2D 形状 [B, C, h, w] 以兼容滑动窗口
                curr_feat_2d = curr_enc_flat.transpose(1, 2).reshape(B, self.hidden_dim, h, w)
                
                # C. 调整密度图分辨率对齐当前层
                if i == 0:
                    curr_density = F.interpolate(densityMap, size=(h, w), mode='bilinear', align_corners=False)
                else:
                    curr_density = F.interpolate(densityMap, size=(h, w), mode='nearest')
                
                # D. 🌟 调用核心模块：ADRE 进行大逃杀过滤与自注意力增强！
                # 进去的是全图 2D 特征，出来的是被物理洗礼过（背景变0，前景融合）的 2D 特征
                feat_2d_new = self.adre_encoders[i](curr_feat_2d, curr_density, adaptive_mu)
                
                # E. 重新展平，准备拼接 [B, C, h, w] -> [B, h*w, C]
                feat_new_flat = feat_2d_new.flatten(2).transpose(1, 2)
                enhanced_feats.append(feat_new_flat)
            ...
            # 🚀 [调试位置]：在拼接前打印，确认各层形状
            # =========================================================
            for i, f in enumerate(enhanced_feats):
                # 这里预期应该是 [BatchSize, 对应层的L, 256]
                print(f"Level {i} ADRE处理后形状: {f.shape}") 
            # =========================================================
            # 将处理后的特征拼回完整的 memory
            memory = torch.cat(enhanced_feats, dim=1)
            
            # 把 adaptive_mu 存下来，如果后续 DQS 还需要，可以直接用！
            self.current_adaptive_mu = adaptive_mu

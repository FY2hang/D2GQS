...
class DetSolver(BaseSolver):    
    ...
    def fit(self, cfg_str):
        ...
        if self.last_epoch > 0:
            ...
            if test_stats: 
                ...
                metrics = {k: (test_stats[k][0] if isinstance(test_stats[k], (list, tuple)) else test_stats[k]) for k in test_stats}
    ...
    def save_best_model(self, test_stats, best_stat, epoch):
        if test_stats:
            ...
            # 创建一份隔离镜像，将密度图拓扑指标从最佳模型海选赛中彻底剥离
            eval_stats = {k: v for k, v in test_stats.items() if k != 'density_metrics'}
            
            # 1. 🌟 纯净的 TensorBoard 记录与海选
            for k in eval_stats: 
                if self.writer and dist_utils.is_main_process(): 
                    if not isinstance(eval_stats[k], (list, tuple)):
                        continue  
                    for i, v in enumerate(eval_stats[k]):
                        if self.evaluator.using_tiny_metrice:
                            self.writer.add_scalar(f'Test/{k}_{coco_name_tiny_list[i]}', v, epoch)
                        else:
                            self.writer.add_scalar(f'Test/{k}_{coco_name_list[i]}', v, epoch)  
   
                all_metrics.append(eval_stats[k][0]) 
                metric_names.append(k)    
            
            # 2. 🌟 单独针对我们的 DPDL 拓扑指标记录 TensorBoard，不影响大局
            if 'density_metrics' in test_stats and self.writer and dist_utils.is_main_process():
                self.writer.add_scalar('Test/Density_SSIM', test_stats['density_metrics'][0], epoch)
                self.writer.add_scalar('Test/Density_PSNR_dB', test_stats['density_metrics'][1], epoch)

            # 计算平均指标 (此时只有纯净的检测 AP 参与计算)
            avg_metric = sum(all_metrics) / len(all_metrics) if all_metrics else 0    
            ...
            # 日志输出
            current_log = dict(zip(metric_names, all_metrics))
            if 'density_metrics' in test_stats:
                current_log['Density_SSIM'] = test_stats['density_metrics'][0]
                current_log['Density_PSNR'] = test_stats['density_metrics'][1]
                
            logger.info(f'Current metrics: {current_log}')  
            logger.info(f'Current avg: {avg_metric:.4f}, Best avg: {best_stat["avg_metric"]:.4f} (epoch {best_stat["epoch"]})') 
